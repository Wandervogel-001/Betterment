...
class MarathonService:
    ...
    async def start_marathon(self, guild: discord.Guild, teams: List[Team]) -> Dict:
        ...
        is_active = await self.team_manager.team_service.get_marathon_state(guild.id)
        if is_active:
            return {"error": "Marathon is already active for this guild"}

        # Get the marathon category if one is set
        category_id = await self.team_manager.team_service.get_setting_field(guild.id, "category", "marathon_category")
        category = None
        if category_id:
            category = discord.utils.get(guild.categories, id=category_id)
            if not category:
                logger.warning(f"Marathon category ID {category_id} set but category not found in guild {guild.id}")

        report = {"created_roles": [], "created_channels": [], "skipped_teams": []}

        for team in teams:
            # 1. Validate the team using the public method from TeamManager
            valid_members, has_leader = await self.team_manager.validator.get_valid_team_members(guild, team.members)
            ...
            role, channel = await self._provision_team_resources(guild, team, valid_members, category)
        ...
        if report["created_roles"] or report["created_channels"]:
            await self.team_manager.team_service.set_marathon_state(guild.id, True)
    ...
    async def end_marathon(self, guild: discord.Guild) -> Dict:
        ...
        teams = await self.team_manager.get_all_teams(guild.id)
        ...
        await self.team_manager.team_service.set_marathon_state(guild.id, False)
    ...
    async def _provision_team_resources(
        ...
        try:
            ...
            if not channel:
                ...
                # Create channel in the specified category if available
                channel = await guild.create_text_channel(
                    team.channel_name,
                    overwrites=overwrites,
                    category=category,
                    reason=f"Marathon channel for {team.team_role}"
                )
    ...
    async def _deprovision_team_resources(
        ...
        return deleted_role, deleted_role
