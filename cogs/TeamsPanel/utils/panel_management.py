...
class PanelManager:
    ...
    def __init__(self, bot, team_manager, marathon_service):
        self.bot = bot
    ...
    async def refresh_team_panel(self, guild_id: int, interaction: Interaction = None):
        ...
        panel_data = await self.team_manager.team_service.get_team_panel(guild_id)
        if not panel_data:
            if interaction:
        ...
        try:
            ...
            if not channel:
                await self.team_manager.team_service.delete_team_panel(guild_id)
                return

            msg = await channel.fetch_message(panel_data["message_id"])
            embed = await self.build_teams_embed(guild_id)
            view = MainPanelView(self.team_manager, self.marathon_service, self)
