...
from .services.team_service import TeamDatabaseService
...
from .utils.panel_management import PanelManager
from .utils.profile_parsing import ProfileParser
...
class TeamsCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.team_service = TeamDatabaseService(self.db)

        # --- Singletons (owned by cog) ---
        self.team_manager = TeamManager(self.team_service)

        # --- Lightweight services/managers ---
        self.permission_manager = PermissionManager()
        self.marathon_service = MarathonService(self.team_manager)
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        if str(payload.emoji) != REACTION_EMOJI:
            return
        communication_channel_id = await self.team_service.get_communication_channel_id(payload.guild_id)
        if not communication_channel_id:
            return
        if payload.channel_id != communication_channel_id:
            return
    ...
    async def create_panel(self, interaction: discord.Interaction):
        ...
        existing = await self.team_service.get_team_panel(interaction.guild_id)
        if existing:
            try:
                ...
                await self.team_service.delete_team_panel(interaction.guild_id)
    ...
    async def manual_save(self, interaction: Interaction, user: Member, timezone: str = None, goals: str = None, habits: str = None):
        ...
        await self.team_service.save_unregistered_member(interaction.guild.id, str(user.id), member_data, role_type)
