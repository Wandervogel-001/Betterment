...
class EditChannelNameModal(Modal, title="Edit Team Channel Name"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await self.team_manager.update_team_channel_name(
...
class DeleteMemberModal(Modal, title="Remove Member(s) from Team"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            team = await self.team_manager.get_team(interaction.guild.id, self.team_role)
...
class TeamFormationModal(Modal, title="Confirm New Team Formation"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.defer(thinking=True, ephemeral=True)
        try:
            unassigned_doc = await self.team_manager.team_service.get_unregistered_document(interaction.guild_id)
            leaders, members = [], []
