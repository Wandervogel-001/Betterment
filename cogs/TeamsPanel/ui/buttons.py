...
class ViewTeamButton(TeamButton):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            teams = await self.team_manager.get_all_teams(interaction.guild_id)
            if not teams:
...
class StartMarathonButton(TeamButton):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            if await self.team_manager.is_marathon_active(interaction.guild.id):
                return await interaction.followup.send("⚠️ Marathon is already active for this server.", ephemeral=True)
            teams = await self.team_manager.get_all_teams(interaction.guild.id)
            if not teams:
            ...
            if results['created_roles'] or results['created_channels']:
...
class EndMarathonButton(TeamButton):
    ...
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True, ephemeral=True)
        try:
            if not await self.team_manager.is_marathon_active(interaction.guild.id):
            ...
            await interaction.followup.send(embed=self._build_results_embed(results), ephemeral=True)
            if results['removed_channels'] or results['processed_teams']:
                await self.panel_manager.refresh_team_panel(interaction.guild_id)
        except Exception as e:
            await self.handle_error(interaction, e)
...
class ReflectButton(TeamButton):
    ...
    """
    def __init__(self, team_manager, panel_manager):
        super().__init__(team_manager=team_manager, panel_manager=panel_manager, label="Reflect & Form Teams", style=discord.ButtonStyle.secondary, custom_id="reflect_button", row=0)
    @moderator_required
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True, ephemeral=True)
        try:
            ...
            view = ReflectionActionsView(self.team_manager, self.panel_manager) if report.get("unassigned_leader_count", 0) + report.get("unassigned_member_count", 0) > 0 else discord.ui.View()
...
class FetchDataButton(TeamButton):
    ...
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True, ephemeral=True)
        try:
            results = await self.team_manager.fetch_server_teams(interaction.guild)
...
class DeleteMemberButton(TeamButton):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            team = await self.team_manager.get_team(interaction.guild_id, self.team_role)
            if not team.members:
...
class EditChannelNameButton(TeamButton):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            await self.team_manager.get_team(interaction.guild_id, self.team_data["team_role"])
...
class ConfirmDeleteButton(TeamButton):
    ...
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True, ephemeral=True)
        try:
            success = await self.team_manager.delete_team_and_resources(interaction.guild, self.team_name)
...
class AssignMemberButton(TeamButton):
    ...
    def __init__(self, team_manager, panel_manager):
        super().__init__(team_manager=team_manager, panel_manager=panel_manager, label="Assign Member", style=discord.ButtonStyle.success, custom_id="assign_member_button")
    @moderator_required
    async def callback(self, interaction: discord.Interaction):
        try:
            from .views import UnregisteredMemberDropdownView # Avoid circular import
            unregistered_doc = await self.team_manager.team_service.get_unregistered_document(interaction.guild_id)
...
class FormTeamButton(TeamButton):
    ...
    def __init__(self, team_manager, panel_manager):
        super().__init__(team_manager=team_manager, panel_manager=panel_manager, label="Form New Teams", style=discord.ButtonStyle.primary, custom_id="form_teams_button")
    @moderator_required
    async def callback(self, interaction: discord.Interaction):
        try:
            await interaction.response.send_modal(TeamFormationModal(self.team_manager, self.panel_manager))
        except Exception as e:
            await self.handle_error(interaction, e)
