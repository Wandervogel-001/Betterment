...
class MainPanelView(View):
    ...
    def __init__(self, team_manager, marathon_service, panel_manager, timeout: Optional[float] = None):
...
class TeamDropdown(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            team = await self.team_manager.get_team(interaction.guild_id, selected_team_name)
...
class UnregisteredMemberDropdown(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        all_teams = await self.team_manager.get_all_teams(interaction.guild_id)
...
class ReflectionActionsView(View):
    def __init__(self, team_manager, panel_manager, timeout: float = 300):
        super().__init__(timeout=timeout)
        self.add_item(AssignMemberButton(team_manager, panel_manager))
        self.add_item(FormTeamButton(team_manager, panel_manager))
