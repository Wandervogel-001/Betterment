...
class MainPanelView(View):
    ...
    async def from_db(cls, embed_service, embed_sender, panel_manager, guild_id, page=0, selected_embed=None, attached_channels=None):
        """Factory method to create MainPanelView with data loaded from the database."""
...
class PrevPageButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        new_view = await MainPanelView.from_db(
...
class DeleteEmbedButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Build new embed and view (reset selection and stay on current page)
            ...
            new_view = await MainPanelView.from_db(
                ...
                interaction.guild.id,
                page=self.view.page,
                selected_embed=None,  # Reset selection after deletion
                attached_channels=[]  # Reset attached channels
            )

            await interaction.response.edit_message(embed=new_embed, view=new_view)

            # Refresh the main panel if it exists
...
class ChannelAttachmentSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Clear existing attachments and add new ones (same logic as original)
        ...
        new_view = await MainPanelView.from_db(
            ...
            page=self.view.page,
            selected_embed=self.selected_embed,
            attached_channels=chosen_channels  # Pass the new attached channels directly
        )
...
class PrefilledRenameEmbedModal(discord.ui.Modal, title="Rename Embed"):
    def __init__(self, embed_service, panel_manager, current_name: str):
        ...
        self.current_embed_name = discord.ui.TextInput(
            label="Current Embed Name",
            default=current_name,
            required=True
        )
        self.add_item(self.current_embed_name)

        self.new_embed_name = discord.ui.TextInput(
            label="New Embed Name",
            placeholder="Enter the new name for the embed",
            required=True
        )
        self.add_item(self.new_embed_name)
